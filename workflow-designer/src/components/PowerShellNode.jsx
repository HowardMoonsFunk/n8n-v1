...
const PowerShellNode = ({ data, id }) => {
  const [showPanel, setShowPanel] = useState(false);
  const [tab, setTab] = useState('config');
  const [active, setActive] = useState(data.active ?? true);
  const [nodeName, setNodeName] = useState(data.label || 'PowerShell Script');
  const [description, setDescription] = useState(data.description || '');
  const [script, setScript] = useState(
    data.script || `# Example PowerShell Script
      ...
      >
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            />
            <h3 className="font-semibold text-gray-700 text-sm">{nodeName}</h3>
          </div>
